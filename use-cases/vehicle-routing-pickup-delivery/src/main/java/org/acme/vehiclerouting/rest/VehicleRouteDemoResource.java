...
public class VehicleRouteDemoResource {
    ...
    public enum DemoData {
        ...
        DemoData(long seed, int shipmentCount, int vehicleCount, LocalTime vehicleStartTime,
            ...
            if (shipmentCount < 1) {
                throw new IllegalStateException(
                        "Number of visitCount (%s) must be greater than zero.".formatted(shipmentCount));
            }
            if (vehicleCount < 1) {
                throw new IllegalStateException(
    ...
    public VehicleRoutePlan build(DemoData demoData) {
        ...
        AtomicLong shipmentSequence = new AtomicLong();
        ...
        Supplier<Shipment> shipmentSupplier = () -> {
            ...
            Shipment shipment = new Shipment(String.valueOf(shipmentSequence.incrementAndGet()), demand.nextInt());
        ...
        List<Shipment> shipments = Stream.generate(shipmentSupplier)
                .limit(demoData.visitCount)
                .toList();

        List<Visit> visits = shipments.stream()
                .flatMap(s -> Stream.of(s.getPickupVisit(), s.getDeliveryVisit()))
                .toList();
