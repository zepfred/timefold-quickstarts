...
import org.acme.sportsleagueschedule.rest.exception.ErrorInfo;
import org.acme.sportsleagueschedule.rest.exception.ScheduleSolverException;
...
public class SportsLeagueSchedulingResource {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(SportsLeagueSchedulingResource.class);
    ...
    public SportsLeagueSchedulingResource() {
        this.solverManager = null;
        this.solutionManager = null;
    }

    @Inject
    public SportsLeagueSchedulingResource(SolverManager<LeagueSchedule, String> solverManager,
    ...
    public String solve(LeagueSchedule problem) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("analyze")
    public ScoreAnalysis<HardSoftScore> analyze(LeagueSchedule problem,
    ...
    public LeagueSchedule getSchedule(
        ...
        LeagueSchedule schedule = getScheduleAndCheckForExceptions(jobId);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("{jobId}/status")
    public LeagueSchedule getStatus(
            @Parameter(description = "The job ID returned by the POST method.") @PathParam("jobId") String jobId) {
        LeagueSchedule schedule = getScheduleAndCheckForExceptions(jobId);
        SolverStatus solverStatus = solverManager.getSolverStatus(jobId);
        return new LeagueSchedule(schedule.getScore(), solverStatus);
    }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("{jobId}")
    public LeagueSchedule terminateSolving(
            @Parameter(description = "The job ID returned by the POST method.") @PathParam("jobId") String jobId) {
